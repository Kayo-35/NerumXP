...
BEGIN
    ...
    if(cd_tipo_meta_var < 7) then
        if(cd_tipo_meta_var > 4) then
            ...
            if(
                somatorio_vl_meta_var / categorias_somatorio_var * 100 <> (select pc_progresso from metas where cd_meta = cd_meta_param) OR (select pc_progresso from metas where cd_meta = cd_meta_param) IS NULL
            ) then
                update metas 
            ...
            -- Atualizando o progresso da meta se necessário
            if(
                somatorio_vl_meta_var <> (select vl_valor_progresso from metas where cd_meta = cd_meta_param) OR 
                (select vl_valor_progresso from metas where cd_meta = cd_meta_param) IS NULL
            ) then
                update metas 
...
drop procedure spGerenteMetas;
select * from metas where cd_meta = 29;
call spGerenteMetas(29);